"""Wrapper macro to make the nextjs rules more ergonomic.
...
def next(
        ...
        next_export_out = "out",
        **kwargs):
    """Generates nextjs targets build, dev & start targets.
    ...
    next(
        ...
        data = [
            ...
            "//nextjs:node_modules/react-dom",
            "//nextjs:node_modules/react",
            "//nextjs:node_modules/typescript",
            "next.config.js",
            "package.json",
        ],
    ...
    in an `nextjs/BUILD.bazel` file where the root `BUILD.bazel` file has
    ...
    ```
    bazel build //nextjs:next_export
    ```
    ...
    Args:
        ...
        srcs: Source files to include in build & dev targets.
            Typically these are source files or transpiled source files in nextjs source folders
        ...
        next_build_out: The `next build` output directory. Defaults to `.next` which is the nextjs default output directory for the `build` command.

        next_export_out: The `next export` output directory. Defaults to `out` which is the nextjs default output directory for the `export` command.
    ...
    js_run_binary(
        ...
        # trying to write to an input. See https://github.com/vercel/nextjs/issues/43344.
        # TODO: fix in nextjs (https://github.com/vercel/nextjs/issues/43344) or find work-around.
        tags = tags + ["manual"],
        **kwargs
    )
